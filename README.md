...
4. Scripts should be able to communicate with STDIN and STDOUT to combine scripts with pipes. STDIN and STDOUT should be treated as SMILES strings. Currently, it was implemented in all scripts, where this is relevant. 
5. All scripts can contain errors, so use them on your own risk. If you find a mistake please create the issue, we will fix it. However, we constantly revise old scripts and fix errors because every found mistake is penultimate.
...
| `get_contact_scaffold`          | Extract contact-anchored ligand scaffolds from SDF poses using ProLIF.                     |
