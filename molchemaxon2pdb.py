...
def main():
    ...
    if input_fname is None:
        with tempfile.NamedTemporaryFile('wt', suffix='.smi', delete=False) as tmp:
            tmp.write(sys.stdin.read())
            tmp_fname = tmp.name
        input_fname = tmp_fname

    if args.preserve_coord and input_fname.split('.')[-1].lower() != 'sdf':
        raise ValueError('Please use --preserve_coord argument only for input SDF format.')

    try:
        calc(input_fname, args.output if args.output is not None else os.getcwd(), args.ncpu, args.field_name, args.pH,
